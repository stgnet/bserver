...
func TestScriptsDisabledFlag(t *testing.T) {
	dir := setupMinimalSite(t, map[string]string{
		"index.yaml": "main:\n - greet\n",
		"greet.yaml": "^greet:\n  tag: div\n  script: python\n  code: \"print('hello')\"",
	})
	...
	if !strings.Contains(output, "script execution disabled") {
		t.Error("should contain 'script execution disabled' comment when scripts are disabled")
	}
}
...
func BenchmarkRenderYAMLPage(b *testing.B) {
	...
	b.ResetTimer()
	for i := 0; i < b.N; i++ {
		renderYAMLPage(docRoot, indexPath, false, 1, false, nil)
	}
}
...
func BenchmarkRenderMarkdownPage(b *testing.B) {
	...
	b.ResetTimer()
	for i := 0; i < b.N; i++ {
		renderMarkdownPage(docRoot, mdPath, false, 1, false, nil)
	}
}
